...
var DesktopCapture = Backbone.Model.extend({
  defaults: {
    ...
    screen_size_id: 1,
    desktop_sizes: [
      {"id": 1,"label": '1440x810(16:9)',"width":1440,'height':810},
...
var DesktopCaptureModalView = Backbone.View.extend({
  ...
  initialize: function (args) {
    //this.getAllPeers();
    this.listenTo(this.model,'open_capture_modal',this.openModal);
    this.listenTo(this.model,'close_modal',this.closeModal);
  },
  events: {
    'click .modal-desktop-fullscreen': 'setFullScreen',
    'click .modal-desktop-capture-hide': 'closeModal'
  },
  ...
  openModal: function(name,src) {
    console.log('Screen OpenModal!!!!!!!!',name,src);
    console.log(this.$el);
    console.log(this.$el.css('display'));
    if (this.$el.css('display') !== 'block') {
      $('#modal-desktop-name').text(name);
      $('#modal-desktop-view').attr('src',src);
